...
<div id="delete-modal" class="fixed inset-0 z-40 hidden items-center justify-center bg-black/50 backdrop-blur-sm flex">
    <div class="bg-white rounded-2xl shadow-2xl p-7 w-full max-w-sm mx-4">
        <div class="flex items-center gap-4 mb-4">
            ...
            <div>
                <h3 class="text-base font-semibold text-gray-800">Delete Department?</h3>
        ...
        <p class="text-sm text-gray-600 mb-6 bg-red-50 border border-red-100 rounded-xl px-4 py-3">
            Kya aap sach mein is department ko permanently delete karna chahte hain?
        </p>
...
<nav class="flex items-center gap-2 text-xs text-gray-400 mb-6">
    ...
    <span class="text-gray-600 font-medium">Departments</span>
</nav>
    ...
    <div class="flex items-center gap-4">
        <div class="w-12 h-12 rounded-2xl bg-violet-600 flex items-center justify-center shadow-lg shadow-violet-200">
            <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5v-2a4 4 0 00-5-3.87M9 20H4v-2a4 4 0 015-3.87m6 5.87a4 4 0 10-8 0m8 0a4 4 0 01-8 0M15 7a4 4 0 11-8 0 4 4 0 018 0z"/>
            </svg>
        </div>
        <div>
            <h1 class="text-xl font-bold text-gray-800">Department Management</h1>
            <p class="text-gray-400 text-xs mt-0.5">Manage all registered departments</p>
        </div>
    </div>
    <a href="{{ route('department.create') }}"
        class="inline-flex items-center gap-2 bg-violet-600 hover:bg-violet-700 text-white text-sm font-semibold
               px-5 py-2.5 rounded-xl transition shadow-sm shadow-violet-200 self-start sm:self-auto">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
...
<div class="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-4">
        <p class="text-xs text-gray-400 uppercase tracking-wider mb-1">Total Departments</p>
...
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-4 mb-4 flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
    <div class="relative w-full sm:w-80">
        ...
        <input type="text" id="search-input" placeholder="Search by Department Name or Status..."
    ...
    <div class="flex items-center gap-3">
        <select id="status-filter" onchange="filterTable()"
            class="text-sm border border-gray-200 bg-gray-50 rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-violet-500 transition">
        ...
        <table class="min-w-full text-sm text-left">
            <thead>
                <tr class="bg-gray-50 border-b border-gray-100">
                    ...
                    <th class="px-5 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">Department Name</th>
            ...
            <tbody id="department-table-body" class="divide-y divide-gray-50">
                <tr>
...
<script>
    ...
    // ─── Update Stats ─────────────────────────────────────────
    function updateStats(depts) {
        document.getElementById('stat-total').textContent    = depts.length;
        document.getElementById('stat-active').textContent   = depts.filter(d => d.status === 'Active').length;
        document.getElementById('stat-inactive').textContent = depts.filter(d => d.status === 'Inactive').length;
    }

    // ─── Render Table ─────────────────────────────────────────
    function renderTable(departments) {
        const tbody = document.getElementById('department-table-body');
        document.getElementById('total-count').textContent = `${departments.length} result${departments.length !== 1 ? 's' : ''}`;
                ...
                <tr>
                    <td colspan="4" class="px-5 py-14 text-center">
                        <div class="flex flex-col items-center gap-3">
                            ...
                            <p class="text-sm text-gray-400 font-medium">Koi department nahi mila</p>
                            <a href="/department/create" class="text-xs text-violet-500 hover:text-violet-700 transition">+ Pehla department add karein</a>
                        </div>
                    </td>
                </tr>`;
            return;
            ...
            <tr class="hover:bg-violet-50/30 transition group">
                ...
                <td class="px-5 py-4">
                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold
                        ...
                        <span class="w-1.5 h-1.5 rounded-full ${dept.status === 'Active' ? 'bg-green-500' : 'bg-red-400'}"></span>
                        ${dept.status}
                    </span>
                </td>
                <td class="px-5 py-4 text-center">
                    <div class="flex items-center justify-center gap-2 opacity-70 group-hover:opacity-100 transition">
                        <a href="/department/create?id=${dept.id}"
                            class="inline-flex items-center gap-1.5 bg-violet-50 hover:bg-violet-100 text-violet-700
                        ...
                        <button onclick="openDeleteModal(${dept.id})"
    ...
    function filterTable() {
        ...
        const filtered = allDepartments.filter(d =>
            (!q || d.name.toLowerCase().includes(q) || d.status.toLowerCase().includes(q)) &&
    ...
    async function fetchDepartments() {
        try {
            ...
            if (res.success) {
                ...
                updateStats(allDepartments);
                renderTable(allDepartments);
            } else {
                showToast('Departments load karne mein error aaya.', 'error');
            }
        } catch {
            showToast('Server se connect nahi ho saka.', 'error');
            document.getElementById('department-table-body').innerHTML = `
                <tr><td colspan="4" class="px-5 py-8 text-center text-red-400 text-sm">Data load nahi ho saka.</td></tr>`;
        }
    }
    ...
    document.getElementById('confirm-delete-btn').addEventListener('click', async function () {
        ...
        try {
            ...
            if (res.success) {
                ...
                showToast(res.message || 'Delete nahi ho saka.', 'error');
            }
        } catch {
            showToast('Server se connect nahi ho saka.', 'error');
        } finally {
